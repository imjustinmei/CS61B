...
public class Model extends Observable {
        ...
        for (int c = 0; c < _board.size(); c++) {
            ...
            for (int r = _board.size() - 2; r > -1; r--) {
                ...
                for (int i = r + 1; i < _board.size(); i++) {
                    Tile check = _board.tile(c, i);
                    if (check == null) fr++;
                    else {
                        if (check.value() == t.value()) {
                            if (merged == 0) {
                                merged = i;
                                _score += t.value() * 2;
                                _board.move(c, i, t);
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        int lastValue = b.tile(b.size()-1, 0).value();
        if (lastValue == b.tile(b.size()-1, 1).value() || lastValue == b.tile(b.size()-2, 0).value()) return true;
        return false;
    }
